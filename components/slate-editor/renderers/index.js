...
export function renderMarkButton(type, icon, editor) {
	...
	return (
		<Button reversed active={isActive} onMouseDown={(e) => onclickMark(e, type, editor)}>
			<Icon>{icon}</Icon>
		</Button>
	);
}
...
const onclickBlock = (e, type, editor) => {
	e.preventDefault();
...
export const renderBlockButton = (type, icon, editor) => {
	...
	return (
		<Button active={isActive} reversed onMouseDown={(e) => onclickBlock(e, type, editor)}>
			<Icon>{icon}</Icon>
		</Button>
	);
};
